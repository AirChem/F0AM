...
PhotolysisScaleFactor = 1;

% Initialization
nj = 71; %number of rate constants. Old value = 70 when no furfural included
Jnames = cell(nj,1);
...
QY{i} = 1; % Assume for 254 nm lights
...
i=i+1;
Jnames{i} = 'J58';  % FURFURAL -> Products...
CS{i} = 'Cross_Section_FURFURAL.csv';
QY{i} = 0.6;

i=i+1;
Jnames{i} = 'J59';  % Benzaldehyde -> Products...
CS{i} = 'Cross_Section_Benzaldehyde.csv';
QY{i} = 0.1;
...
for i=1:nj
    ...
    if strcmp('J11',Jnames{i})==1 || strcmp('J12',Jnames{i})==1
        J.(Jnames{i}) = IntegrateJ(CS{i},QY{i},LFlux,T,P,[],plotem)*PhotolysisScaleFactor;
    end
